...
const InternshipApplicationModal = ({ internship, onClose, onSuccess }) => {
  const [formData, setFormData] = useState({
    coverLetter: 'I am very interested in this internship and believe my skills are a great match.',
    skills: [],
    experience: '',
    education: ''
  });
      ...
      setTimeout(async () => {
        try {
          ...
          if (resumeResponse.data && resumeResponse.data.id) {
            ...
            // Keep the latest record's id/path
            setUploadedResumeId(resumeData.id);
            setUploadedResumePath(resumeData.filePath || '');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await internshipAPI.applyForInternship(internship.id, currentUser.id, applicationData);
    ...
    <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
      <div className="relative top-20 mx-auto p-5 border w-full max-w-2xl shadow-lg rounded-md bg-white">
        ...
        <div className="bg-blue-50 rounded-lg p-4 mb-6">
          <h3 className="font-semibold text-blue-800">{internship.company}</h3>
          <p className="text-sm text-blue-600 mt-1">{internship.location} • {internship.duration} months • Stipend: {internship.stipend}</p>
        </div>
          ...
          <div>
            ...
            {!hasResume && !isLoadingResume && fetchAttempted && (
              <p className="text-sm text-yellow-600 mt-2 flex items-center">
                No resume found. You can upload one or add skills manually.
              </p>
            )}
          </div>
